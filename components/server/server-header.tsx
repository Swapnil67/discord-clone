  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="focus:outline-none" asChild>
        ...
        >
          {server.name}
          <ChevronDown className="h-10 w-5 ml-auto" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-56 text-xs font-medium dark:text-neutral-400 space-y-[2px]">
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={() => onOpen("deleteServer", { server })} className="app-danger px-3 py-2 text-sm cursor-pointer">
            Delete Server
        ...
        {!isAdmin && (
          <DropdownMenuItem 
           ...
           className="app-danger px-3 py-2 text-sm cursor-pointer">
            Leave Server
